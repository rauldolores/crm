...
description: Generate Supabase SQL migrations at deploy time from the session branch diff. Load this when your developer dispatch asks you to generate the deploy-time migration (it points you at the shared <base>/simple worktree). This is NOT a feature ticket — the workflow below replaces the normal ticket rules: here you DO write SQL under apps/crm/supabase/migrations/, and you do not edit TS/TSX/CSS.
---
...
here: writing SQL under `apps/crm/supabase/migrations/` is the whole job. Do not edit any
...
- Entity TypeScript types (e.g. `apps/crm/src/**/types.ts`, resource type defs).
...
1. Find this session's own prior migration(s):
   `git diff --diff-filter=A session-base/<SESSION_SHORT_ID>..session/<SESSION_SHORT_ID> -- apps/crm/supabase/migrations/`
...
**Surface note.** Technical-harness writes to `apps/crm/supabase/migrations/`. On the web-chat
surface (CRM Builder) the launcher stages migrations in `apps/crm/supabase/migrations-pending/`
and promotes them to `apps/crm/supabase/migrations/` after the user confirms: follow the
...
Write to `apps/crm/supabase/migrations/<YYYYMMDDHHMMSS>_<SESSION_SHORT_ID>_migration_<slug>.sql`
...
`apps/crm/supabase/schemas/03_views.sql`, recreate the view with **`CREATE OR REPLACE
...
Then update `apps/crm/supabase/schemas/03_views.sql` to the **same order** — the
...
```bash
cd <WORKTREE_PATH> && git add apps/crm/supabase/migrations && git commit -m "migration(<SESSION_SHORT_ID>): <slug>"
```
...
- Re-emitting SQL already present in `apps/crm/supabase/migrations/`.

## Verification

- [ ] Diff taken from `session-base/<SESSION_SHORT_ID>..session/<SESSION_SHORT_ID>`.
- [ ] Every emitted statement is an incremental delta not already in `apps/crm/supabase/migrations/`.
