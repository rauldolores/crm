...
describe("TaskCreateSheet", () => {
  it("creates a task for a selected contact and updates last_seen", async () => {
    ...
    await typeOptions.getByText("Llamada").click();
    ...
    await expect.element(screen.getByText("Tarea añadida")).toBeInTheDocument();

    await expect
      .element(screen.getByText("Crear tarea"))
